...
export class UserAuthService {
  ...
  async createNewUser(req, profileImage) {
    try {
      ...
      const destFilePath = path.join(
        srcPath,
        `${userId}_avatar${path.extname(profileImage.filename)}`,
      );
      ...
      body.profileImage = `192.168.1.27:3000/avatars/${userId}_avatar${path.extname(
  ...
  async updateUser(userId: string, userData: User, profileImage): Promise<any> {
    try {
      ...
      if (profileImage) {
        ...
        userData.profileImage = `192.168.1.27:3000/avatars/${userId}_avatar${path.extname(
          profileImage.filename,
        )}`;
      }
      let item_to_udate = {
