...
export class UserAuthService {
  ...
  async createNewUser(req,profileImage) {
    try {
      ...
      if (profileImage) {
        body.profileImage = `http:148.212.195.49:3000/avatars/${profileImage.filename}`;
      }
      ...
      body.profileImage = `http:148.212.195.49:3000/avatars/${userId}_avatar.jpg`;
  ...
  async updateUser(
    ...
    try {
      ...
      if (profileImage) {
        userData.profileImage = `http:148.212.195.49:3000/avatars/${profileImage.filename}`;
        const srcPath = path.join(__dirname, '../..','src', 'assets', 'avatars');
        const sourceFilePath = path.join(srcPath, profileImage.filename);
        const destFilePath = path.join(srcPath, `${userId}_avatar${path.extname(profileImage.filename)}`);
        ...
        userData.profileImage = `http:148.212.195.49:3000/avatars/${userId}_avatar.jpg`;
